...
void GLWidget::paintGL()
{
    ...
    if(coordinatesScale.rows()>1)
    {
        ...
        if(contourSettingObject.meshVisible==true){PaintLine();}
        if(contourSettingObject.deformedMeshVisible==true){PaintLineDeform();}
        if(contourSettingObject.resultVisible==true){PaintElement();}
...
{
    Vector_Node.resize(numberOfNodes);
    Vector_NodeColor.resize(numberOfNodes);
    //Data for Node
...
void GLWidget::InitializeNode()
{
    ...
    VertexBuffer_Node.allocate(&Vector_Node[0],numberOfNodes*sizeof(Vector_Node[0])); //Allowcate memory, link to Vecto_node;
...
void GLWidget::PaintNode()
{
    ...
    {
        VertexArray_Node.bind();
        glDrawArrays(GL_POINTS,0,numberOfNodes); //Draw nodes
        VertexArray_Node.release();
    }
    ShaderProgram_Node->release();
}

void GLWidget::ElementData()
{    
    //Using deformation coordinates
    Vector_Element.resize(0);
    Vector_ElementColor.resize(0);
    int numberOfFaces=4*elementInfor.tet10Count+12*elementInfor.hex20Count+12*elementInfor.hex8Count+
            6*elementInfor.pyra5Count+6*elementInfor.pyra13Count+8*elementInfor.prism6Count+8*elementInfor.prism15Count;

    Vector_Element.reserve(numberOfFaces); //8-faces
    Vector_ElementColor.reserve(numberOfFaces);
    double nodeVal;
    MatrixXd nodeColor=MatrixXd::Zero(8,3);
    MatrixXi nodeIndex=MatrixXi::Zero(8,1);
    MatrixXi faceIndex;
    ...
    for(int i=0;i<numberOfElements;i++)
    {
        ...
        int eleIndex=elementInfor.elmementIndex(i,0);
        int nodeCount=elements(eleIndex,11);

        //Calculate on nodes
        for (int j=0;j<8;j++) //maximum is 8 node
        {
            if(nodeCount==10 && j==4){break;}
            if((nodeCount==5 || nodeCount==13) && j==5){break;}
            if((nodeCount==6 || nodeCount==15) && j==6){break;}
            ...
            if(shownData.rows()==1)
            {
                if(shownData(0,0)==1)
                {
                    nodeVal=elements(eleIndex,12);
                    contourSettingObject.noc=elements.col(12).maxCoeff();
                }
                else if(shownData(0,0)==2)
                {
                    nodeVal=elements(eleIndex,13);
                    contourSettingObject.noc=elements.col(13).maxCoeff();
                }
            }
            else
            {
                nodeVal=shownData(nodeIndex(j,0),0);
            }
            CalculateRGB_Color(minimumValue,maximumValue,nodeVal);
            nodeColor(j,0)=redColor;
            nodeColor(j,1)=greenColor;
            nodeColor(j,2)=blueColor;
        }

        //Get node indices for each face
        if(nodeCount=8 || nodeCount==20)
        {
            faceIndex.resize(12,3);
            faceIndex<<1,4,3,
                    1,3,2,
                    5,1,2,
                    5,2,6,
        ...
        else if (nodeCount==5 || nodeCount==13)
        {
            faceIndex.resize(6,3);
            faceIndex<<1,2,5,
                    2,3,5,
                    3,4,5,
                    4,1,5,
        ...
        else if(nodeCount==6 || nodeCount==15)
        {
            faceIndex.resize(8,3);
            faceIndex<<1,2,3,
                    4,5,6,
                    1,3,4,
                    4,3,6,
...
void GLWidget::InitializeElement()
{
...
void GLWidget::InitializeLineDeform()
{
...
void GLWidget::LineData()
{
    Vector_Line.resize(0);
    Vector_LineDeform.resize(0);

    Vector_Line.reserve(12*2*elementInfor.hex20Count+12*2*elementInfor.hex8Count+6*2*elementInfor.tet10Count+
                        8*2*elementInfor.pyra5Count+8*2*elementInfor.pyra13Count+9*2*elementInfor.prism6Count+9*2*elementInfor.prism15Count);
    Vector_LineDeform.reserve(12*2*elementInfor.hex20Count+12*2*elementInfor.hex8Count+6*2*elementInfor.tet10Count+
                              8*2*elementInfor.pyra5Count+8*2*elementInfor.pyra13Count+9*2*elementInfor.prism6Count+9*2*elementInfor.prism15Count);

    unsigned int nodeA, nodeB;
    MatrixXd nodeIndex=MatrixXd::Zero(8,1);
    MatrixXi lineIndex;
    for (int i=0;i<numberOfElements;i++)
    {
        int eleIndex=elementInfor.elmementIndex(i,0);
        ...
        if(nodeCount==8 || nodeCount==20)
        {
            lineIndex.resize(12,2);
            nodeIndex(0,0)=elements(eleIndex,1)-1;
            nodeIndex(1,0)=elements(eleIndex,2)-1;
            nodeIndex(2,0)=elements(eleIndex,3)-1;
            nodeIndex(3,0)=elements(eleIndex,4)-1;
            nodeIndex(4,0)=elements(eleIndex,5)-1;
            nodeIndex(5,0)=elements(eleIndex,6)-1;
            nodeIndex(6,0)=elements(eleIndex,7)-1;
            nodeIndex(7,0)=elements(eleIndex,8)-1;
            lineIndex<<1,2,2,3,3,4,4,1,5,6,6,7,7,8,8,5,1,5,2,6,3,7,4,8;
        }
        else if (nodeCount==10)
        {
            lineIndex.resize(6,2);
            nodeIndex(0,0)=elements(eleIndex,1)-1;
            nodeIndex(1,0)=elements(eleIndex,2)-1;
            nodeIndex(2,0)=elements(eleIndex,3)-1;
            nodeIndex(3,0)=elements(eleIndex,4)-1;
            lineIndex<<1,2,2,3,3,1,4,1,4,2,4,3;
        }
        else if (nodeCount==5 || nodeCount==13)
        {
            lineIndex.resize(8,2);
            nodeIndex(0,0)=elements(eleIndex,1)-1;
            nodeIndex(1,0)=elements(eleIndex,2)-1;
            nodeIndex(2,0)=elements(eleIndex,3)-1;
            nodeIndex(3,0)=elements(eleIndex,4)-1;
            nodeIndex(4,0)=elements(eleIndex,5)-1;
            lineIndex<<1,2,2,3,3,4,4,1,5,1,5,2,5,3,5,4;

        }
        else if (nodeCount==6 || nodeCount==15)
        {
            lineIndex.resize(9,2);
            nodeIndex(0,0)=elements(eleIndex,1)-1;
            nodeIndex(1,0)=elements(eleIndex,2)-1;
            nodeIndex(2,0)=elements(eleIndex,3)-1;
            nodeIndex(3,0)=elements(eleIndex,4)-1;
...
void GLWidget::GetShownData(Ref<MatrixXd> coordinatesScale, Ref<MatrixXd> coordinatesDeform, Ref<MatrixXd> elements, Ref<MatrixXd> shownData, ElementInfor elementInfor)
{
    this->coordinatesScale=coordinatesScale;
    this->coordinatesDeform=coordinatesDeform;
    this->elements=elements;
    this->shownData=shownData;
    this->elementInfor=elementInfor;
    numberOfNodes=coordinatesDeform.rows();
    numberOfElements=elementInfor.elmementIndex.rows();
}
...
void GLWidget::GetStepResultInformation(BaseStepResult stepResultObject)
{
    ...
    {
        this->stepResultObject.maxValue=shownData.maxCoeff();
        this->stepResultObject.minValue=shownData.minCoeff();
    }
}
